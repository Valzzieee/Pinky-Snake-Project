...
function gambarLoading(waktu) {
  ...
  ctx.fillText("PINKY SNAKE", LEBAR / 2, yJudul);
...
function gambarStart() {
  ...
  ctx.fillText("⚡ Glow: efek cahaya   🧊 Freeze: perlambat   🍄 Poison: kontrol kanan-kiri terbalik", LEBAR / 2, tombol.y + tombol.h + 25 + 64 * skalaFont());
}
...
function loop(waktuSekarang) {
  ...
  if (layarAktif === "loading") {
    ...
    const speedNormal = Math.min(SPEED_MAKS, SPEED_AWAL + Math.floor(skor / SKOR_PER_KENAIKAN));
    let speed = Math.min(SPEED_MAKS, shiftPressed ? speedNormal + 6 : speedNormal);

    if (efekAktif && efekAktif.tipe === "freeze") {
      speed = Math.max(3, Math.floor(speed * 0.05));
    }
